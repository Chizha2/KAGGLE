...
#file = pd.read_csv("../zadanie/kuip_train.csv", na_values = "NA") # чтение файла, пустые значения = "NA"
...
#x_train, x_test, y_train, y_test = train_test_split(file.drop(columns = ['LUX']), file['LUX'], test_size = 0.40) # разделение на 4 части (2 тестовых и 2 валидационных)

# x_train = file.head(280)
...
y_predict = y_scaler.inverse_transform(predictions)
...
print("Linear")
mae_linear=int(mean_absolute_error(y_real, y_predict))
rmsle_linear = rmsle(y_real, y_predict)
print(mae_linear)
print(rmsle_linear)

print("SGD")
y_predict_sgd = y_scaler.inverse_transform(modelsgd.predict(x_test))
mae_sgd=int(mean_absolute_error(y_real, y_predict_sgd))
rmsle_sgd = rmsle(y_real, y_predict_sgd)
print(mae_sgd)
print(rmsle_sgd)
